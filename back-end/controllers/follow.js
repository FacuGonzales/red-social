...
function saveFollow(req, res){
    ...
    follow.save((err, followStored) => {
        ...
        if(!followStored) return res.status(404).send({message: 'Error al seguir usuario'})
...
function deleteFollow(req, res){
    ...
    Follow.find({'user': userId, 'followed': followId}).remove( err => {
        ...
        return res.status(200).send({message: 'Se dejo de seguir correctamente.'});
    })
}
...
// Obtener listados sin paginar 
function getMyFollows(req, res){
    var userId = req.user.sub;

    // Obtiene los que yo sigo
    var find = Follow.find({user: userId});

    // Obtiene los que me siguen
