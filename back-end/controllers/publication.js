...
function probando(req, res){
    res.status(200).send({message: 'Ingresando a publication'});
};

function savePublication(req, res){
    var params = req.body;

    if(!params.text) return res.status(200).send({message: 'Se debe enviar un texto'});
...
function getAllPublicationsUser(req, res){
    var page = 1;
...
function deletePublication(req, res){
    ...
    Publication.findOne({"user": req.user.sub, "_id":publicationId}).remove( (err, publicationRemove) => {
    ...
    if(req.files){
        ...
        if(file_ext == 'png'|| file_ext == 'jpg'|| file_ext == 'jpeg'|| file_ext == 'gif'){
            ...
            Publication.findOne({"user": req.user.sub, "_id":publicationId}).exec((err, publication) => {
                ...
                if(publication){
                    // Actualizar documentro de la publicacion
                    Publication.findByIdAndUpdate(publicationId, {file: file_name}, {new: true}, (err, publicationUpdate) => {
                        if(err) return res.status(500).send({message: 'Error en la peticion'});
        
                        if(!publicationUpdate) return res.status(404).send({message: 'No se ha podido actualizar la pulblicacion'});
        
                        return res.status(200).send({publication: publicationUpdate});
                    
                    });
                }else{
                    return removeFileOfUpload(res, files_path, 'No tienes permisos para actualizar esta publicacion');
                }
            });
